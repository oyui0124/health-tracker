...
## カロリー・栄養素について
- Web検索結果が提供されている場合は、必ずその情報を優先して使用する
- ブランド商品の場合は「📌公式値:」と表記
- 一般食品の場合は「📊成分表:」と表記
- 推定の場合は「🔄推定:」と表記
...
export async function POST(req: NextRequest) {
  try {
    ...
    // コンテキスト情報とWeb栄養検索を並列取得
    const [todaySummary, recentHistory, nutritionContext] = await Promise.all([
      getTodaysSummary(),
      getRecentHistory(),
    ...
    const contextInfo = `${todaySummary}\n${recentHistory}${nutritionContext}`;
