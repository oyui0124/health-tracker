...
## カロリー・栄養素の推定ルール
- ブランド名・商品名（例：「ファミチキ」「セブンの鮭おにぎり」「マックのビッグマック」「スタバのフラペチーノ」）が含まれる場合は、その商品の公式栄養成分情報をもとに正確に計算する。公式値を知っている場合は「公式値:」と明記する
- チェーン店名（例：「松屋の牛めし」「すき家」「CoCo壱」「サイゼリヤ」）が含まれる場合も同様に、そのメニューの公式カロリー・PFCで計算する
- 一般的な食品（例：「ごはん」「味噌汁」）の場合は、日本食品標準成分表ベースで一般的な分量から推定し、「成分表ベース:」と明記する
- 自炊・手作りの場合は材料と分量を確認し、各食材ごとに積み上げて計算する
- 推定の確度を3段階で示す：📌公式値（ブランド商品）、📊成分表ベース（一般食品）、🔄推定（情報不足時）
